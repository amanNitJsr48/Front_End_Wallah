...
### Note:
* microservices works on seperation of concern
* it works on single responsibility principal. where each service has its own responsibility
* first Page loads then componennt get rendered and then finally useEffect get called
* fetch method is the is the power of browser its not JS method
